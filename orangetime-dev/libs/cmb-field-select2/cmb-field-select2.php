...
define( 'PW_SELECT2_URL', get_template_directory_uri() . '/libs/cmb-field-select2/' );
...
function pw_multiselect_escape( $check, $meta_value ) {
	$return_value = is_array($meta_value) ? implode( ',', $meta_value ) : $meta_value;
	return ! empty( $meta_value ) ? $return_value : $check;
}
